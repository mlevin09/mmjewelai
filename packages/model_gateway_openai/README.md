...
The adapter implements only the transient `ImageGenerationExecutor` contract. It intentionally has
no metadata-only `generate()` method, because a successful production call must not discard the only
copy of generated bytes before durable private-object staging.
